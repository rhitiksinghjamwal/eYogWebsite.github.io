...
<head>
	...
	<!----css file link-->
	<link rel="stylesheet" type="text/css" href="css/yoga_audios.css">
...
<body id="mybody" class="bg-white">
	...
	<div>
		<center style="height: 50px; background-color: deepskyblue; ">
			<p class="text-white h3" style="">Practice Audio Guided <?php echo $_GET['course_name']; ?> Free</p>
		</center><br><br><br><br>
	</div>
		...
		<center>
			<div class="row">
				<section class="col-md-7 mt-4">
					...
					<iframe style="border-radius:12px" width="100%" height="380" <?php
																					$_SESSION['aid'] = $_GET['audio_id'];
																					$audio_id = $_GET['audio_id'];
																					$sql = "select * from audios where audio_id='$audio_id'";
																					$result = mysqli_query($con, $sql);
																					while ($row = mysqli_fetch_array($result)) {
																					?> src=<?php echo $row['audio_path'];   //fetching youtube video path from database & storing into src attribute
																					}
					...
					<div class='commentdiv shadow bg-light' style="border:1px #d2c8c8 solid; background-color: #dfe1e4;">
						<?php
						$audio_id = $_GET['audio_id'];    //getting the value of video is from the previous page using GET
						echo "      
				<form method='POST' action='" . setComments($con) . "'>
					<input type='hidden' name='uid' value='Anonymous'>
					<input type='hidden' name='vid' value='" . $audio_id . "'>
					...
					<table class="table table-hover mt-3 table-borderless bg-light">
						<tbody>
							<div class="card-header text-center bg-light">
								<h5><strong>Popular Audios</strong></h5>
							</div>
							...
							$sql = "select * from audios where course_name='$course_name'";
							$result = mysqli_query($con, $sql);
							...
							?>
								...
								<tr>
									<td><a href=""><img src=../../uploadimg/thumbnail/<?php echo $row['audio_image']; ?> height="100" width="150"></a></td>
